...
use crate::domain::{
    build_variable_pool, substitute_request, EnvironmentScope, Request, ResponseBody,
    ResponseBodyView, Variable, VariableLayers,
};
use crate::transport::{block_on, send_http_request, HttpRequestBody, HttpResponse};
...
impl ApiHelperApp {
    ...
    fn variable_pool_for_source(
        ...
        build_variable_pool(VariableLayers {
    ...
    pub(super) fn resolve_request_variables(
        ...
        let pool = self.variable_pool_for_source(collection_index, folder_index, &request.variables);
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            cx.update(|window, app| {
                this.update(app, |app, cx| {
                    app.finish_request(tab_id, result, window, cx);
                    cx.notify();
                })
                .ok();
            })
    ...
    fn finish_request(
        ...
        match result {
            Ok(response) => {
                ...
                tab.response_status_text = Some(response.status_text);
                tab.response_elapsed_ms = Some(response.elapsed_ms);
